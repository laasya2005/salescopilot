    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-5">
          <div className="flex items-center gap-3 mb-2">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${result.worthChasing ? "bg-emerald-500/20" : "bg-red-500/20"}`}>
              {result.worthChasing ? (
            ...
            <div>
              ...
              <p className={`text-lg font-bold ${result.worthChasing ? "text-emerald-400" : "text-red-400"}`}>
                {result.worthChasing ? "YES" : "NO"}
              </p>
            </div>
          </div>
          <p className="text-xs text-slate-500">{result.worthChasingReasoning}</p>
        </div>
        ...
        <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-5 flex flex-col items-center">
          <ScoreRing
            ...
            label="Lead Score"
            color={result.leadScore >= 70 ? "#10b981" : result.leadScore >= 40 ? "#f59e0b" : "#ef4444"}
          />
          <p className="text-xs text-slate-500 mt-1 text-center">{result.leadScoreReasoning}</p>
        </div>
        ...
        <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-5 flex flex-col items-center justify-center">
          ...
          <p className="text-xs text-slate-500 mt-3 text-center">{result.dealRiskReasoning}</p>
        </div>
        ...
        <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-5 flex flex-col items-center">
          <ScoreRing
            ...
            color={result.closeForecast >= 60 ? "#10b981" : result.closeForecast >= 30 ? "#f59e0b" : "#ef4444"}
          />
          <p className="text-xs text-slate-500 mt-1 text-center">{result.closeForecastReasoning}</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={() => navigator.clipboard.writeText(result.followUpEmail)}
      ...
      {/* AI Voice Coaching */}
      <div className="bg-gradient-to-r from-slate-900/80 to-violet-950/30 rounded-xl border border-violet-500/20 p-6">
        ...
        {!audioUrl ? (
          <button
            ...
            className="bg-violet-600 hover:bg-violet-500 disabled:bg-slate-600 text-white font-medium py-2.5 px-5 rounded-lg transition-all duration-200 flex items-center gap-2 text-sm"
          >
            {voiceLoading ? (
              <>
